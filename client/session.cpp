...
void Session::processBuffer()
{
    while (running)
    {
        ...
        if (packet.find("Main,") == 0)
        {
            // Extrair IP e porta da string
            size_t commaPos = packet.find(',', 5); // A partir do quinto caractere, procurar a próxima vírgula
            if (commaPos != std::string::npos)
            {
                std::string ip = packet.substr(5, commaPos - 5);
                int port = std::stoi(packet.substr(commaPos + 1));

                // Chamar a função setServer
                //std::cout << "Main server found at " << ip << ":" << port << std::endl;
                client.setMainServer(ip, port);
            }
        }
    ...
    void Session::transmitPacket(twt::PacketType type, std::string payload)
    {
        ...
        if (nacks == 3)
            std::cout << "Não foi possível se conectar ao servidor, tente novamente dentro de alguns instantes" << RED << RESET << std::endl;
    ...
    void Session::printYourMessages()
    {
        while (true)
        {
            ...
            std::cout << "> " << msg.content << " (" << msg.timestamp << ")" << std::endl
                      << std::endl;
        }
    }
